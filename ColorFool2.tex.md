...
where $\gamma( \cdot )$ converts the intensities of an image from RGB to the Lab color space, ${N_k}^{a} \in \mathcal{N_k}^a$ and ${N_k}^{b} \in \mathcal{N}_k}^b$ are the adversarial perturbations in the channels $\textit{a}$ and $\textit{b}$ that are chosen randomly from the set of natural-color ranges, $\mathcal{N_k}^a$ and   $\mathcal{N_k}^b $ in the $\textit{a}$ and $\textit{b}$ channels. 
